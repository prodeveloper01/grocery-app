...
class AdminController extends Controller
{
  ...
  public function orderdetails(Request $request)
  {
    ...
    if($request->user_id == 1){
        ...
        $status=Order::select('order.driver_id','order.user_id','order.address','order.landmark','order.building','order.pincode','order.order_type','order.tax','order.promocode','order.discount_amount','order.order_number','order.status','order.order_notes','order.delivery_charge','order.lat','order.lang','users.name',\DB::raw("CONCAT('".url('/public/images/profile/')."/', users.profile_image) AS profile_image"),'users.mobile')->where('order.id',$request['order_id'])
        ...
        @$order_total = array_sum(array_column(@$data, 'item_price'));
